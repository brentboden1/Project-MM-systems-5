...
namespace LobbyService.Web
{
    ...
    public class Service1 : IService1
    {
           ...
           // state.ActivePlayer = tmp.ActivePlayer;
            ...
            foreach(var p in tmp.PlayerList)
            {
                if(p.MyPlayer.PlayerId == me.PlayerId)
                {
                    ...
                    {
                        state.OwnedProps.Add(new DTO.MonopolyEngine.PropertyToClient() { HouseLevel = item.HouseLevel, ID = item.ID, Morguage = item.Morguage, SetLevel = item.SetLevel });
                    }
                }
            }
        ...
        public void EscapePrison(DTO.Player me)
        {
            if (NewGame.Started)
            {
                NewGame.EscapePrison(me);
            }
        }

        public string GetGamePlayerByOrder(int ID)
        {
            string temp = null;
            if (NewGame.Started)
               temp = NewGame.GetPlayerName(ID);
            return temp;
        }

        public void BuyHouse(DTO.Player me, byte ID)
        {
            if (NewGame.Started)
            {
                NewGame.BuyHouse(me,ID);
            }
        }
